...
public class StopWordsRemover extends SparkMLFeatureBase implements RealTimeTransformer {
    ...
    public Dataset select(Dataset<Row> transformedData) {
        return transformedData.select("FILTERED");
    }

    @Override
    public String[] methods() {
        return new String[]{"stop_words_remover", "stop_words_remover_transformer"};
    }
    ...
    public ModelLogicPlan load(SparkSession session, String alias, String path) {
        return new ModelLogicPlan(session.read().parquet(path).alias(alias).logicalPlan(),
            org.apache.spark.ml.feature.StopWordsRemover.load(path));
    }
