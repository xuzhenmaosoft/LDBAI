...
public class IndexToString extends SparkMLFeatureBase implements RealTimeTransformer {
    ...
    public Dataset select(Dataset<Row> transformedData) {
        return transformedData.select("ORIGINALCATEGORY");
    }

    @Override
    public String[] methods() {
        return new String[]{"index_to_string", "index_to_string_transformer"};
    }
    ...
    public ModelLogicPlan load(SparkSession session, String alias, String path) {
        return new ModelLogicPlan(session.read().parquet(path).alias(alias).logicalPlan(),
            org.apache.spark.ml.feature.IndexToString.load(path));
    }
}
