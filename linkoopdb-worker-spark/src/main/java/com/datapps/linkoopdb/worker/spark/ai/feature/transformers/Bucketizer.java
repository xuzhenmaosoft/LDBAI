...
public class Bucketizer extends SparkMLFeatureBase {
    ...
    public String[] methods() {
        return new String[]{"bucketizer", "bucketizer_transformer"};
    }
        ...
        for (int i = 0; i < parameters.length; i++) {
            if (arguments[i] != null && pipelineStage.hasParam(parameters[i])) {
                ...
                if (argument instanceof Object[]) {
                    if (parameters[i].equalsIgnoreCase("splitsArray")) {
                        Object[] objArras = (Object[]) argument;
                        int length = objArras.length;
                        double[][] arra = new double[length][];
                        for (int k = 0; k < length; k++) {
                            Object[] objArra = (Object[]) objArras[k];
                            int length1 = objArra.length;
                            arra[k] = new double[length1];
                            for (int j = 0; j < length1; j++) {
                                arra[k][j] = Double.parseDouble((objArra[j]).toString());
                            }
                        }
    ...
    public PipelineStage createSimplePipelineStage() {
        return new org.apache.spark.ml.feature.Bucketizer();
    }
    ...
    public ModelLogicPlan load(SparkSession session, String alias, String path) {
        return new ModelLogicPlan(session.read().parquet(path).alias(alias).logicalPlan(),
            org.apache.spark.ml.feature.Bucketizer.load(path));
    }
}
