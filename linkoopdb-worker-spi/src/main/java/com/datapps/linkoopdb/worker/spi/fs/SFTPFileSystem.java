...
public class SFTPFileSystem extends FileSystem {
    ...
    private Path makeAbsolute(Path workDir, Path path) {
        if (path.isAbsolute() && path.toUri().getPath().startsWith(workDir.toString())) {
            return path;
        }
        try {
            URI uri = path.toUri();
            URI workDirUri = new URI(uri.getScheme(), uri.getAuthority(), workDir.toUri().getPath(), null, null);
            return new Path(workDirUri.toString(), removeHead(uri.getPath()));
        } catch (URISyntaxException e) {
            throw new RuntimeException("Unable to make absolute", e);
        }
    }
    ...
    /**
     * Convenience method, so that we don't open a new connection when using this method from within another method. Otherwise every API invocation incurs the
     * overhead of opening/closing a TCP connection.
     *
    ...
    private FileStatus[] listStatus(FTPClient client, Path file)
        ...
        Path workDir = new Path(client.printWorkingDirectory());
        Path absolute = makeAbsolute(workDir, file);
        FileStatus fileStat = getFileStatus(client, file);
        if (fileStat.isFile()) {
        ...
        for (int i = 0; i < ftpFiles.length; i++) {
            fileStats[i] = getFileStatus(ftpFiles[i], file);
        }
        return fileStats;
    }
        ...
        if (ftpFiles != null) {
            for (FTPFile ftpFile : ftpFiles) {
                if (ftpFile.getName().equals(file.getName())) { // file found in dir
                    fileStat = getFileStatus(ftpFile, file.getParent());
                    break;
                }
            }
            if (fileStat == null) {
    ...
    private boolean rename(FTPClient client, Path src, Path dst)
        ...
        if (!parentSrc.equals(parentDst)) {
            ...
            if (parentSrc.contains(parentDst)) {
                renamedFlag = client.rename(absoluteSrc.toUri().getPath(), absoluteDst.toUri().getPath());
            }
